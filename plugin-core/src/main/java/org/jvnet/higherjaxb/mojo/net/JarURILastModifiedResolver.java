...
/**
 * A Jar {@link URI} last modified resolver.
 */
	...
	protected Long getLastModifiedForScheme(URI uri)
	{
		...
		try
		{
			...
			getLogger().debug(format(
				"Retrieving the last modification timestamp of the URI [%s] via the main URI [%s].", uri, mainURI));

			lastModified = getParent().getLastModified(mainURI);
		}
		catch (Exception ex)
		{
			getLogger().error(format("Could not retrieve the main URI from the Jar URI [%s].", uri), ex);
